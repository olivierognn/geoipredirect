                ...
                foreach ($allStores as $store) {
                    ...
                    if ($country && $countries && strpos($countries, $country) !== false
                        ...
                        if ($storeId != $currentStoreId) {
                            $addStoreCodeToUrl = $this->addStoreCodeToUrl($storeCode, $addToUrl);
                            
                            if (substr($redirectStoreUrl, -1) == '/') {
                                $redirectStoreUrl = substr($redirectStoreUrl, 0, -1) . $addStoreCodeToUrl;
                            } else {
                                $redirectStoreUrl .= $addStoreCodeToUrl;
                            }
...
//        $ip = '213.184.225.37';//Minsk
        if ($this->getDbConfig('amgeoip/import/location')
            && $this->getDbConfig('amgeoip/import/block')
            && $this->getDbConfig('amgeoip/import/block_v6')
        ) {
            $geoIpHelper = new Amasty_Geoip_Helper_Data();
            ...
            if ($isIpv6) {
                ...
                $longIP = $geoIpHelper->getLongIpV6($ip);
...
/*            $pathInfo = explode('//', $path);
    ...
    public function getDbConfig($path, $scopeId = 0, $default = null)
    {
        ...
        if (is_string($value) && strpos($value, '{{') !== false) {
            if (strpos($value, '{{unsecure_base_url}}') !== false) {
                $unsecureBaseUrl = $this->getDbConfig(Mage_Core_Model_Store::XML_PATH_UNSECURE_BASE_URL, $scopeId);
                $value = str_replace('{{unsecure_base_url}}', $unsecureBaseUrl, $value);
            } elseif (strpos($value, '{{secure_base_url}}') !== false) {
                $secureBaseUrl = $this->getDbConfig(Mage_Core_Model_Store::XML_PATH_SECURE_BASE_URL, $scopeId);
    ...
    protected function applyLogic($request)
    {
        ...
        if ($this->redirectAllowed && $result == "") {
            $result = $requestUri;
        }
